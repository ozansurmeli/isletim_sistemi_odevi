...
cikti = open("SJF_sonuc.txt", "w", encoding="utf-8")
print("SJF (Non-Preemptive) GANTT:", file=cikti)
...
while tamamlanan < n:
    ...
    ready = []
    for i in range(n):
        pid, arrival, burst, priority = processler[i]
    ...
    if len(ready) == 0:
        ...
        for i in range(n):
            if not bitti_mi[i]:
                if processler[i][1] < sonraki_gelis:
                    sonraki_gelis = processler[i][1]
    ...
    sec = ready[0]
    for i in ready[1:]:
        # karşılaştırma: burst küçük olan kazanır
        a = processler[i]
        b = processler[sec]
        if (a[2], a[1], a[0]) < (b[2], b[1], b[0]):
            sec = i

    pid, arrival, burst, priority = processler[sec]
